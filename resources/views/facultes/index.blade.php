...
@section('content')
    ...
    <x-page-header title="Facultés" subtitle="Gestion des facultés de l'université."
        :breadcrumb="['Paramètres académiques' => null, 'Facultés' => null]">
        <x-slot:actions>
            <x-btn :href="route('facultes.create')">
    ...
    <x-card padding="p-0">
        @if ($facultes->isEmpty())
            <x-empty message="Aucune faculté enregistrée pour le moment." />
        @else
                            ...
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                <td class="px-5 lg:px-6 py-3.5">
                                    <div class="flex items-center gap-3">
                                        <div class="w-9 h-9 rounded-xl bg-[#7f1d1d]/10 text-[#7f1d1d] flex items-center justify-center font-bold text-xs">
                                            {{ mb_substr($faculte->nom_faculte, 0, 2) }}
                                        </div>
                                        <span class="font-semibold text-slate-700">{{ $faculte->nom_faculte }}</span>
                                    </div>
                                </td>
                                <td class="px-5 lg:px-6 py-3.5 text-center text-slate-600">{{ $fmt($faculte->promotions_count) }}</td>
                                <td class="px-5 lg:px-6 py-3.5 text-center text-slate-600">{{ $fmt($faculte->etudiants_count) }}</td>
                                <td class="px-5 lg:px-6 py-3.5">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ route('facultes.edit', $faculte) }}" title="Modifier" class="w-9 h-9 rounded-xl flex items-center justify-center text-slate-400 hover:text-[#991b1b] hover:bg-[#991b1b]/5 transition-colors">
                                        ...
                                        <form method="POST" action="{{ route('facultes.destroy', $faculte) }}" onsubmit="return confirm('Supprimer cette faculté ?');">
                                            @csrf
                                            @method('DELETE')
